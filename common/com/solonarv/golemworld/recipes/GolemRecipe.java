...
public abstract class GolemRecipe {
    ...
    protected Block[] midRow;
    protected Block[] botRow;
    protected Block shoulderBlock;
    protected boolean smart;
    
    protected class EntityPos{
        public double posX;
        public double posY;
        public double posZ;
    ...
    protected EntityPos checkRecipeInWorld(int x, int y, int z, World world){
        ...
        if(tarBlock==this.midRow[1].blockID){
            ...
            if(p!=-1) return new EntityPos(x,y-1,z,p);
        }
        if(tarBlock==this.botRow[1].blockID){
            p=this.checkFromPumpkin(x, y+2, z, world);
            if(p!=-1) return new EntityPos(x,y,z,p);
        }
